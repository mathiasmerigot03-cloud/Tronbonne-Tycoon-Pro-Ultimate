...
<h1>🏢 Tronbonne Tycoon Pro Ultimate</h1>
...
<div id="gameDiv">
  ...
  <div class="card">
    ...
    💰 Argent : <span id="argent">0</span> €<br>
    📦 Trombonnes : <span id="trombonnes">0</span> / <span id="stockMax">10</span><br>
    👷 Employés : <span id="employees">0</span><br>
    📈 Revenus cumulés : <span id="revenuTotal">0</span> €<br>
    💲 Prix trombonne actuel : <span id="pricePerItem">10</span> €
    <div class="bar"><div id="stockBar" class="bar-fill"></div></div>
  </div>

  <div class="card">
    <button onclick="collect()">📎 Collecter une trombonne</button>
    <button onclick="sell()">💰 Vendre toutes les trombonnes</button>
    <button onclick="upgrade()">🏗️ Augmenter capacité (+5) – 50€</button>
    <button onclick="hire()">👷 Embaucher un employé – 200€</button>
    <button onclick="invest()">📈 Investissement aléatoire – 100€</button>
...
function updateLeaderboard(){
  ...
  users.slice(0,5).forEach(u=>{
    ...
    div.innerHTML=`${u.pseudo}: ${u.revenu}€`;
...
function login(){
  ...
  updateUI(); startAutoProduction(); notify("Connecté !","success"); addHistory("Connexion réussie"); saveUser();
}
...
function sell(){
  if(user.trombonnes===0) return notify("Rien à vendre !","error");
  ...
  addHistory(`Vente: ${user.trombonnes} trombonnes → +${revenue}€`);
  user.trombonnes=0; fluctuatePrice(); updateUI(); saveUser(); notify(`Vendu ! +${revenue}€`,"success"); updateChart();
}

function upgrade(){
  if(user.argent<50) return notify("Pas assez d'argent !","error");
  user.argent-=50; user.stockMax+=5; addHistory("Capacité augmentée de 5"); updateUI(); saveUser(); notify("Capacité augmentée","success");
}

function hire(){
  if(user.argent<200) return notify("Pas assez d'argent !","error");
  user.argent-=200; user.employees++; addHistory("Nouvel employé embauché"); updateUI(); saveUser(); notify("Employé embauché","success");
}
...
function invest(){
  ...
  if(chance<0.2){ notify("Échec de l'investissement !","error"); addHistory("Investissement raté"); return;}
  const gain=Math.floor(Math.random()*300); user.argent+=gain; user.revenuTotal+=gain; user.revenusHistory.push(gain);
  addHistory(`Investissement réussi +${gain}€`); updateUI(); saveUser(); notify(`Investissement réussi +${gain}€`,"success"); updateChart();
}
...
function startAutoProduction(){
  ...
  autoInterval=setInterval(()=>{
    if(user.employees>0){
      ...
      if(toAdd>0){ user.trombonnes+=toAdd; updateUI(); saveUser(); animateCollect(); addHistory(`${toAdd} trombonnes produites par employés`);}
    }
    randomEvents();
  },3000);
}
...
function randomEvents(){
  ...
  else if(chance<0.1){ // commande spéciale
    const bonus=Math.floor(Math.random()*50+20);
    user.argent+=bonus; user.revenuTotal+=bonus; updateUI(); saveUser(); notify(`Commande spéciale ! +${bonus}€`,"success"); addHistory(`Commande spéciale: +${bonus}€`);}
}
...
// Logout
function logout(){
  saveUser(); user=null; document.getElementById('loginDiv').style.display='block'; document.getElementById('gameDiv').style.display='none'; notify("Déconnecté","success"); clearInterval(autoInterval);
}
...
function animateCollect(){
  const el=document.createElement('span'); el.innerText='📎'; el.style.position='absolute';
